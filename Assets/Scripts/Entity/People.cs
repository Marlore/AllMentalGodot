...
namespace Entity.People
{
    ...
    public class Person
    {
        ...
        {
            if (Mental == 1 && Intellegence <= 3)
                Criminal = new Maniac(this);
            Live -= MentalHealth;
        }
        private void Aged()
        {
        ...
        public void FindHome()
        {
            do
            {
                ...
                {
                    var nonBusyApart = PlayerInfo.CurrentCity.CityApartments.FindAll(x => !x.Busy);
                    var apart = nonBusyApart[rand.Next(nonBusyApart.Count)];
        ...
        public void FindJob()
        {
            if (this.Job == null)
            {
                ...
                else if (Age > 21 && Age < 60)
                {
                    ...
                    var potenitalJob = PlayerInfo.CurrentCity.Vacancy.Values.Where(x => x.StatValue <= this.Intellegence && !x.IsBusy).Select(x=>x).ToList();
                    var maxPotentialJob = potenitalJob.Find(x=>x.Salary== potenitalJob.Max(x=>x.Salary) && potenitalJob.Max(x => x.Salary)>= work.Salary);
                    if(maxPotentialJob!=null)
                        work = maxPotentialJob;
                    Job = work;
                    Job.Worker = this.Id;
                }
                else if (Age >= 60)
